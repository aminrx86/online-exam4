/**
 * Storage Manager - مدیریت ذخیره‌سازی با Supabase (نسخه به‌روز)
 * @version 2.1.0
 */
class StorageManager {
    ...
    // ===== متد جدید: همگام‌سازی نتایج از Supabase به LocalStorage =====
    ...
    // ===== آمار نتایج (همیشه از Supabase می‌خواند) =====
    async getResultsStats() {
        // همیشه از Supabase بخوان
        const results = await this.getResults();
        if (results.length === 0) {
