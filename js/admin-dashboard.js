/**
 * Admin Dashboard - پنل مدیریت (نسخه کامل با اصلاحات)
 * @version 4.2.0
 */
...
class AdminDashboard {
    ...
    saveQuestion(e) {
        ...
        if (id) {
            this.questionBank.updateQuestion(parseInt(id), questionData);
            alert('✅ سوال با موفقیت ویرایش شد');
        } else {
            this.questionBank.addQuestion(questionData);
    ...
    deleteQuestion(id) {
        ...
        if (confirm('⚠️ آیا از حذف این سوال اطمینان دارید؟')) {
            this.questionBank.deleteQuestion(id);
            this.loadQuestionsTab();
        ...
        input.onchange = (e) => {
            ...
            reader.onload = (event) => {
                try {
                    ...
                    if (this.questionBank.importQuestions(questions)) {
    ...
    // ========== کارنامه (اصلاح شده با async/await) ==========
